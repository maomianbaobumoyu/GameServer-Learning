...
像这种末尾带const的函数 **返回值 函数名 const** 这里的 `const` 写在**函数参数列表之后、函数体之前**，这是 **C++ 常成员函数**的标志，**修饰整个成员函数**。

普通成员函数：`this` 类型为 `类名* const`（指针本身不可改，指向的对象内容可改）

**常成员函数**：`this` 被强制变为 `const 类名* const`

含义：**当前对象的所有成员变量都不允许被修改**。

函数内部**禁止修改类的任何成员变量**；

函数内部**禁止调用其他非 `const` 成员函数**（因为非 `const` 函数可能修改对象）；

设计意图：这个函数是**只读查询函数**（判断状态、获取数据），不会改动对象。
